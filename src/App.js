...
function Canvas() {
  ...
  const insertDog = useMutation(({ storage, setMyPresence }) => {
    const shapeId = Date.now().toString() + "_dog";
    const shape = new LiveObject({
      x: getRandomInt(300),
      y: getRandomInt(300),
      fill: getRandomColor()
  ...
  const insertHat = useMutation(({ storage, setMyPresence }) => {
    const shapeId = Date.now().toString() + "_hat";
    const shape = new LiveObject({
      x: getRandomInt(300),
      y: getRandomInt(300),
      fill: getRandomColor()
  ...
  return (
    <>
      ...
      <div className="toolbar">
        ...
        <button onClick={() => insertDog()}>Dog</button>
        <button onClick={() => insertHat()}>Hat</button>
...
function LiveImage({ id, onShapePointerDown, type }) {
  const { x, y } = useStorage((root) => root.shapes.get(id));
